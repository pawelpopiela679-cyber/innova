...
function run_seed(): array
{
    ...
    // Czy to naprawdę PIERWSZE uruchomienie (świeża baza), czy tylko kolejny
    // przebieg install.php po aktualizacji kodu (np. nowa funkcja, migracja
    // schematu)? Sprawdzamy PRZED czymkolwiek innym w tej funkcji, więc jeśli
    // choć jedna grupa już istnieje — ktoś (seed przy poprzedniej instalacji
    // ALBO sama właścicielka przez panel) już to skonfigurował.
    //
    // Dlaczego to ważne: przykładowi prowadzący (ola@/kasia@/marek@/ania@/
    // beata@/tomek@innova-pracownia.pl), przykładowe grupy/terminy i
    // przykładowy rodzic/dziecko/zapis mają sens TYLKO przy pierwszym
    // uruchomieniu — dają gotowy, działający przykład zamiast pustej strony.
    // Bez tej blokady każde ponowne odpalenie install.php (a robimy to przy
    // każdej aktualizacji kodu) WSKRZESZAŁO usuniętych wcześniej fikcyjnych
    // prowadzących z powrotem jako żywe konta z domyślnym hasłem — i
    // właścicielka musiała ich za każdym razem ręcznie kasować. Od teraz: raz
    // wgrane dane startowe, potem tylko schemat (ensure_schema) — cała
    // reszta (prowadzący, grupy, grafik) żyje wyłącznie w panelu.
    //
    // UWAGA dla siebie na przyszłość: jeśli kiedyś trzeba będzie dodać NOWY
    // rodzaj zajęć kodem (jak w historii tego projektu — dodanie Matematyki/
    // Eksperymentatorium), ta blokada go pominie na już działającej stronie —
    // taki rodzaj zajęć trzeba będzie dograć osobną, jednorazową migracją,
    // a nie przez rozszerzenie $classTypeDefs niżej.
    $alreadyInitialized = (bool) $pdo->query('SELECT COUNT(*) c FROM class_groups')->fetch()['c'];
    ...
    if (!$alreadyInitialized) {
        ...
        $log[] = 'Przykładowi prowadzący, grupy i grafik: pominięto (strona już skonfigurowana — zarządzasz tym teraz w panelu).';
    }
    ...
    // --- Przykładowy rodzic + dziecko + jeden potwierdzony zapis — TYLKO przy
    // pierwszej instalacji (patrz $alreadyInitialized wyżej), z tego samego
    // powodu co przykładowi prowadzący: po usunięciu tego konta przez
    // właścicielkę nie ma wracać przy każdym kolejnym install.php.
    if (!$alreadyInitialized) {
